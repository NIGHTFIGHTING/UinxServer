...
/**
 * deactivate_nonbloc - 设置 I/O为阻塞模式
 * @fd：文件描述符
 */
 ...
 * @fd套接字
 * @addr：输出参数，返回堆放地址
 * @wait_seconds：等带超时描述 ，如果为0表示正常模式
...
/*一个一个字符的读取数据，这种效率比较低，因为 多次调用 recv /read函数
 静态变量为接收的数据进行缓存，下一次接收的时候，从缓存中看数据是否有\r\n,不可重入的函数*/

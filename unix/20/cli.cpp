...
void do_getlist(int sock) {
	...
	printf("hads %d users logined server\n", ntohl(count));
	client_list.clear();
...
void parse_cmd(char* cmdline, int sock, struct sockaddr_in* servaddr) {
	...
	if(strcmp(cmd, "exit") == 0) {
		...
		if(sendto(sock, &msg, sizeof(msg), 0, (struct sockaddr*)servaddr, sizeof(servaddr)))
			ERR_EXIT("sendto");
		
		printf("user %s has logout serve\n", username);
		exit(EXIT_SUCCESS);
...
void chat_cli(int sock) {
	...
	while(1) {
		...
		if(cmd == S2C_ALREADY_LOGINED)
			printf("user %s already logined server, please another username\n", username);
